...
def _get_preferred_cached_revision(repo):
    if "main" in repo.refs:
        return repo.refs["main"]
    return max(repo.revisions, key=lambda revision: revision.last_modified)
...
def _resolve_cached_model_paths(model_names: dict) -> dict:
    try:
        ...
        repo_id_to_local_paths = {
            repo.repo_id: str(_get_preferred_cached_revision(repo).snapshot_path)
