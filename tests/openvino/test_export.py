...
class ExportModelTest(unittest.TestCase):
    ...
    SUPPORTED_ARCHITECTURES = {
        ...
        in get_supported_model_for_library("transformers")
    ...
    def test_qwen3_asr_cache_prefers_main_revision(self):
        main_revision = Mock(snapshot_path=Path("/cache/main"))
        pull_request_revision = Mock(snapshot_path=Path("/cache/pull-request"))
        cached_repo = Mock(
            repo_id="org/model",
            revisions=[pull_request_revision, main_revision],
            refs={"main": main_revision, "refs/pr/1": pull_request_revision},
        )

        with patch("utils_tests.os.path.exists", return_value=True), patch(
            "utils_tests.scan_cache_dir", return_value=Mock(repos=[cached_repo])
        ):
            resolved = _resolve_cached_model_paths({"qwen3_asr": "org/model"})

        self.assertEqual(resolved["qwen3_asr"], "/cache/main")
